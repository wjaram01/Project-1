...
class Departamento:
    ...
    def mostrarDepartamento(self):
        ...
        archiDepartamentos = archiDepartamentos.leer()
        for i in archiDepartamentos:
            print("{}     {}".format(i[0], i[1]))
        input("Presione una tecl par continuar...")
...
class Administrativo(Empleado):
    ...
    def getEmpleado(self):
        return  [self.id,self.nombre,str(self.departamento.getid),str(self.cargo.id),self.direccion,self.cedula,self.telefono,str(self.fechaIngreso),str(self.sueldo),str(self.comision)]
...
class Obrero(Empleado):
    ...
    def getEmpleado(self):
        return  [self.id,self.nombre,str(self.departamento.getid),str(self.cargo.id),self.direccion,self.cedula,self.telefono,str(self.fechaIngreso),str(self.sueldo),str(self.cc)]
...
class Deduccion:
    ...
    def mostrarDeduccion(self):
        print('Valor Iess     ({})% \nValor comision  ({})% \nValor antiguedad ({})% '.format(self.getIess()*100, self.getComision()*100, self.getAntiguedad()*100))
...
class Sobretiempo:
    ...
    def calcSobretiempo(self):
        return round(self.empleado.valorHora()+(float(self.h50)*1.5+float(self.h100)*2),2)
...
class Nomina:
    ...
    def calcularNominaDetalle(self,empleado,deduccion):
        ...
        self.detalleNomina.append([
             ...
             str(round(rubrosIngresos[0],2)),str(round(rubrosIngresos[1],2)),str(round(rubrosIngresos[2],2)),str(round(rubrosIngresos[3],2)),str(round(rubrosIngresos[4],2)),
             str(round(rubrosEgresos[0])),str(round(rubrosEgresos[1],2)),str(round(rubrosEgresos[2],2)),str(round(rubrosEgresos[3],2))
        ])
...
class DetalleNomima(CalculoRol):
    ...
    def getSobretiempo(self,aamm):
      ...
      if self.empleado.id[0]=="O":
          ...
          archiSobretiempo = archiSobretiempo.buscar2(self.empleado.id, aamm)
          if archiSobretiempo:
              entSobretiempo = Sobretiempo(self.empleado,archiSobretiempo[2], archiSobretiempo[3], archiSobretiempo[4],True,archiSobretiempo[0])
              calSob = entSobretiempo.calcSobretiempo()
              return calSob
      return 0
    def getAntiguedad(self,deduccion):
        calAnt = 0
        if self.empleado.id[0]=="O":
            ann = self.empleado.fechaIngreso[:-6]
            ann = int(ann)
            calAnt = (round(deduccion.getAntiguedad() * (date.today().year - ann),2))* self.empleado.sueldo
        return calAnt
    ...
    def getPrestamo(self,aamm):
        ...
        if prestamo:
            entPrestamo = Prestamo(prestamo[1],prestamo[2],float(prestamo[3]),float(prestamo[4]),float(prestamo[5]),prestamo[0])
